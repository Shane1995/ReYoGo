...
export function registerStockMovementsHandlers(): void {
  ipcMain.handle(StockMovementsIPC.GET_CURRENT_STOCK, (_e, entityId?: string, asOfDate?: string) =>
    getRepos().stockMovements.getCurrentStockByItem(entityId, asOfDate),
  );
  ipcMain.handle(
    StockMovementsIPC.GET_WEIGHTED_AVG_COSTS,
    (_e, entityId?: string, asOfDate?: string) =>
      getRepos().stockMovements.getWeightedAvgCosts(entityId, asOfDate),
  );
