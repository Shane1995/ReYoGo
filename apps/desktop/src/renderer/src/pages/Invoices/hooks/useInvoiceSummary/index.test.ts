...
describe('useInvoiceSummary', () => {
  describe('itemsWithCategory', () => {
    ...
    it('uses empty strings when category is not found', () => {
      ...
      const { result } = renderHook(() =>
        useInvoiceSummary([makeLine()], [itemNoCat], [mockCategory], VatMode.Exclusive, 15),
      );
  ...
  describe('invoiceSummary', () => {
    it('computes subtotal, vat, and grandTotal for exclusive vatable lines', () => {
      ...
      const { result } = renderHook(() =>
        useInvoiceSummary([line], [mockItem], [mockCategory], VatMode.Exclusive, 15),
      );
    ...
    it('does not count lines with no itemId toward lineCount', () => {
      ...
      const { result } = renderHook(() => useInvoiceSummary([line], [], [], VatMode.Exclusive, 15));
    ...
    it('sums multiple lines', () => {
      ...
      const { result } = renderHook(() =>
        useInvoiceSummary(lines, [mockItem], [mockCategory], VatMode.Exclusive, 15),
      );
