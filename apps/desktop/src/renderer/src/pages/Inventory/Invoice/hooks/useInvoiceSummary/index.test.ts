...
describe('useInvoiceSummary', () => {
  describe('itemsWithCategory', () => {
    it('attaches category name and type label to each item', () => {
      const { result } = renderHook(() =>
        useInvoiceSummary([makeLine()], [mockItem], [mockCategory]),
      );
    ...
    it('uses empty strings when category is not found', () => {
      ...
      const { result } = renderHook(() =>
        useInvoiceSummary([makeLine()], [itemNoCat], [mockCategory]),
      );
  ...
  describe('invoiceSummary', () => {
    it('computes subtotal, vat, and grandTotal for exclusive lines', () => {
      const line = makeLine({
        quantity: 2,
        vatMode: 'exclusive',
        vatRate: 15,
      ...
      const { result } = renderHook(() => useInvoiceSummary([line], [mockItem], [mockCategory]));
    ...
    it('does not count lines with no itemId toward lineCount', () => {
      ...
      const { result } = renderHook(() => useInvoiceSummary([line], [], []));
    ...
    it('sums multiple lines', () => {
      ...
      const { result } = renderHook(() => useInvoiceSummary(lines, [mockItem], [mockCategory]));
