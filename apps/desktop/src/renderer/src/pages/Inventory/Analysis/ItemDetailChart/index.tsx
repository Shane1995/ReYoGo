...
export function ItemDetailChart({ group, metric }: { group: ItemGroup; metric: Metric }) {
  ...
  const xTicks = Array.from(
    { length: xTickCount },
    (_, i) => new Date(minTime + (i / Math.max(xTickCount - 1, 1)) * (maxTime - minTime)),
  );
      ...
      >
        ...
        {crosshairX !== null && tooltip && (
          <>
            ...
            <circle
              cx={crosshairX}
              cy={yScale(getYValue(tooltip.entry))}
              r={6}
              fill={lineColor}
              stroke="white"
              strokeWidth={2.5}
          ...
          <div className="px-3 py-2 space-y-1.5">
            <div className="flex justify-between">
              ...
              <span className="font-mono font-semibold text-foreground">
                {fmt(tooltip.entry.unitPrice)}
                {tooltip.entry.uom ? ` / ${tooltip.entry.uom}` : ''}
              </span>
            </div>
            <div className="flex justify-between">
            ...
            {tooltip.changeFromPrev !== null && (
              <div className="flex justify-between">
                ...
                <span className={changeCls(tooltip.changeFromPrev, true)}>
                  {fmtPct(tooltip.changeFromPrev)}
                </span>
              </div>
            )}
            {tooltip.changeFromFirst !== null && tooltip.entryIdx > 0 && (
              <div className="flex justify-between">
                ...
                <span className={changeCls(tooltip.changeFromFirst)}>
                  {fmtPct(tooltip.changeFromFirst)}
                </span>
              </div>
            )}
          </div>
